...
class Wavo_Blog_Masonry extends Widget_Base {
    use Wavo_Helper;
    public function get_name() {
        return 'wavo-blog-masonry';
    }
    public function get_title() {
        return 'Blog Masonry (N)';
    }
    ...
    protected function register_controls() {
        ...
        $this->add_control( 'gap',
            [
                ...
                'default' => 15,
                'selectors' => [
                    '{{WRAPPER}} .item--masonry' => 'padding: calc( {{SIZE}}px / 2 );',
                    '{{WRAPPER}} .blog-masonry-row, {{WRAPPER}} .blog--masonry-row' => 'margin: 0 calc( -{{SIZE}}px / 2 );',
                ],
                'separator' => 'before',
            ]
        );
        $this->add_control( 'col',
            [
                ...
                'type' => Controls_Manager::SELECT,
                'options' => [
                    'col-lg-3' => esc_html__( '4 Column', 'wavo' ),
                    'col-lg-4' => esc_html__( '3 Column', 'wavo' ),
                    'col-lg-6' => esc_html__( '2 Column', 'wavo' ),
                    'col-lg-12' => esc_html__( '1 Column', 'wavo' ),
                ],
                'default' => 'col-lg-6',
                'separator' => 'before',
            ]
        ...
        $this->add_control( 'colmd',
            [
                'label' => esc_html__( 'Column ( Desktop )', 'wavo' ),
                'type' => Controls_Manager::SELECT,
                'options' => [
                    'col-md-4' => esc_html__( '3 Column', 'wavo' ),
                    'col-md-6' => esc_html__( '2 Column', 'wavo' ),
                    'col-md-12' => esc_html__( '2-1 Column', 'wavo' ),
                ],
                'default' => 'col-md-6',
            ]
        );
        $this->add_control( 'colsm',
            [
                'label' => esc_html__( 'Column ( Tablet )', 'wavo' ),
                'type' => Controls_Manager::SELECT,
                'options' => [
                    'col-sm-4' => esc_html__( '3 Column', 'wavo' ),
                    'col-sm-6' => esc_html__( '2 Column', 'wavo' ),
                    'col-sm-12' => esc_html__( '1 Column', 'wavo' ),
                ],
                'default' => 'col-sm-6',
            ]
        );
        $this->add_control( 'colxs',
            [
                'label' => esc_html__( 'Column ( Phone )', 'wavo' ),
                'type' => Controls_Manager::SELECT,
                'options' => [
                    'col-xs-6' => esc_html__( '2 Column', 'wavo' ),
                    'col-xs-12' => esc_html__( '1 Column', 'wavo' ),
                ],
                'default' => 'col-xs-12',
            ]
        );
        ...
        $this->start_controls_section( 'post_options_section',
            [
                'label' => esc_html__( 'Post Data Display', 'wavo' ),
                'tab' => Controls_Manager::TAB_CONTENT
            ]
        );
    ...
    protected function render()
    {
        ...
        $classes[] .= 'item--masonry item--'.$attrtype;
        ...
        if ( $the_query->have_posts() ) {
            ...
            // print html
            $is_admin = $is_editor ? 'blog-masonry-row' : 'blog--masonry-row';
            echo '<div class="blog--layout-grid-wrapper"><div id="blog--masonry-'.$id.'" class="row '.$is_admin.'">'.$html.'</div></div>';
            ...
            if ( $is_editor ) { ?>
                ...
                jQuery(document).ready(function ($) {
                    ...
                    /* wavoGalleryIsotope */
                    function wavoMasonryBlog2() {

                        var container = document.querySelector('#blog--masonry-<?php echo $id ?>');
                        var msnry;
